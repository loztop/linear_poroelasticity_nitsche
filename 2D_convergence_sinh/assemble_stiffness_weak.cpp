 ...
 if(xf>0.001)
 {
		...
		for (unsigned int i=0; i<phi_face_f.size(); i++){						
			...
			for (unsigned int i=0; i<phi_face_p.size(); i++){						
						for (unsigned int j=0; j<phi_face_f.size(); j++){											
							#if !THREED
							Kpx(i,j) += -dt*JxW_face_f[qp]*phi_face_p[i][qp]*phi_face_f[j][qp]*normal(0); 
						  Kpy(i,j) += -dt*JxW_face_f[qp]*phi_face_p[i][qp]*phi_face_f[j][qp]*normal(1); 	
							#endif
							
							#if THREED
							Kpx(i,j) += -dt*JxW_face_f[qp]*phi_face_p[i][qp]*phi_face_f[j][qp]*normal(0); 
						  Kpy(i,j) += -dt*JxW_face_f[qp]*phi_face_p[i][qp]*phi_face_f[j][qp]*normal(1); 	
							Kpz(i,j) += -dt*JxW_face_f[qp]*phi_face_p[i][qp]*phi_face_f[j][qp]*normal(2); 	
							#endif
						}					
			}
